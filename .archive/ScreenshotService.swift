...
// SCDisplay, SCWindow, SCContentFilter, SCStreamConfiguration, etc. are not marked Sendable by Apple.
...
class ScreenshotService: NSObject, SelectionWindowDelegate {
    ...
    private var selectionFlowTask: Task<Void, Never>?
    ...
    func selectionWindow(_ window: SelectionWindow, didSelectRect rect: CGRect) {
        ...
        let shouldPostCaptureFlowEnded = true
    ...
    func captureWindowUnderMouse(trigger: CaptureTrigger = .menuBar) {
        captureTrigger = trigger
    ...
    private func beginSelectionSession() -> UUID {
        selectionFlowTask?.cancel()
        selectionFlowTask = nil
    ...
    private func startSelectionFlow(overlayConfiguration: SelectionOverlayView.Configuration) {
        ...
        selectionFlowTask = Task { [weak self] in
            guard let self = self else { return }
            do {
    ...
    /// Detect the application category from a bundle ID.
    /// nonisolated: pure lookup in a static table, no actor state access.
    private nonisolated static func detectFrontmostApp(bundleID: String?) -> AppCategory {
        guard let bundleID else { return .unknown }
        return AppCategory.appCategoryMap[bundleID] ?? .unknown
    }
    ...
    private func saveToDiskAsync(
        ...
        let seq = AppSettings.shared.screenshotSequence
        ...
        // Task.detached: intentionally cuts @MainActor inheritance to run I/O off the main thread.
        // Owner: saveToDiskAsync caller (usually handleSuccessfulCapture on MainActor).
        // Priority: .utility — disk I/O is not user-interactive.
        // Lifetime: immediately awaited (.value), no fire-and-forget.
        // Cancellation: propagated via parent Task (selectionFlowTask), checked via Task.checkCancellation() upstream.
        let (savedPath, finalSeq) = await Task.detached(priority: .utility) {
            ScreenshotService.saveToFile(
    ...
    /// Returns the saved path and the next sequence number (input sequence is consumed by value).
    ...
    private func saveToFileAndGetPath(cgImage: CGImage, pointSize: CGSize) -> String? {
        ...
        let (result, nextSeq) = ScreenshotService.saveToFile(
            cgImage: cgImage,
            pointSize: pointSize,
            imageFormat: imageFormat,
            sequence: seq,
